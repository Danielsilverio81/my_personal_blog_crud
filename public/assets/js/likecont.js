...
const submitForm = async (element, postId, action) => {
  ...
  const deslikesCountElement = document.getElementById('dislikesCount');
  ...
  try {
    ...
    let response = await fetch(`/posts/${postId}/${action}?_method=put`, { headers: headers, body: body, method: 'PUT' });
    ...
    if (response.ok) {
      ...
      if (data.message === 'Post Liked Success') {
        ...
        likesCountElement.textContent = data.likesCount;
          deslikesCountElement.textContent = data.dislikeCount;
        ...
        console.error('Erro ao processar a resposta do servidor:', data.message);
      }
    } else {
